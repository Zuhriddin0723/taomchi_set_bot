...
def get_connection():
    """SQLite bazasiga ulanish qaytaradi. check_same_thread=False thread xavfsizligi uchun."""
...
def init_db():
    """Ma'lumotlar bazasi sxemasini ishga tushiradi va boshlang'ich ma'lumotlarni qo'shadi."""
    conn = get_connection()
    cursor = conn.cursor()
    ...
    CREATE TABLE IF NOT EXISTS orders (
        ...
        user_id       INTEGER,
        order_type    TEXT,        -- 'subscription' yoki 'set'
        set_name      TEXT,
        customer_name TEXT,
        phone         TEXT,
        address       TEXT,
    ...
    CREATE TABLE IF NOT EXISTS food_sets (
        ...
        description TEXT,
        price       INTEGER,
        images      TEXT    -- Vergul bilan ajratilgan mahalliy fayl yo'llari (max 3)
    )
    """)

    # Sozlamalar jadvali
    # key = 'subscription'          → value = matn, value_photo = rasm yo'li
    # key = 'today_dish_text'       → value = matn
    # key = 'today_dish_photo'      → value = rasm yo'li  (value_photo emas!)
    cursor.execute("""
    ...
    # Boshlang'ich obuna matni
    cursor.execute("SELECT COUNT(*) as cnt FROM settings WHERE key = 'subscription'")
    if cursor.fetchone()['cnt'] == 0:
        cursor.execute(
            ...
            (
                "subscription",
                (
                    "📅 Taomchi Set 1 oylik obunasi orqali har kuni mazali taomlar bepul yetkazib beriladi "
                    "va 20% chegirmaga ega bo'lasiz!\n\nObuna narxi: 1,500,000 so'm/oy.\n\nHoziroq buyurtma bering!"
                ),
                None
            )
        )
...
def get_all_users():
    """Barcha foydalanuvchilarni ro'yxatini qaytaradi (eng yangilari birinchi)."""
    conn = get_connection()
    cursor = conn.cursor()
...
def add_order(user_id, order_type, set_name, customer_name, phone, address, latitude=None, longitude=None):
    ...
    cursor.execute(
        """INSERT INTO orders
           (user_id, order_type, set_name, customer_name, phone, address, latitude, longitude, status, created_at)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
...
def delete_food_set(set_id):
    """Setni o'chiradi va o'chiriladigan rasm yo'llarini qaytaradi."""
    conn = get_connection()
    cursor = conn.cursor()
...
def clear_subscription_info():
    """Obuna matni va rasmini o'chiradi (NULL qilib qo'yadi), diskdagi rasmni ham o'chiradi."""
    conn = get_connection()
    cursor = conn.cursor()
    ...
    if row and row['value_photo']:
        try:
            if os.path.exists(row['value_photo']):
                os.remove(row['value_photo'])
        except Exception:
            pass
...
# ─────────────────────────────────────────────
# Bugungi taom funksiyalari  ← TUZATILGAN
# ─────────────────────────────────────────────

def get_today_dish():
    """
    Bugungi taom matnini va rasm yo'lini qaytaradi.
    Matn  → settings WHERE key = 'today_dish_text',  value ustunida
    Rasm  → settings WHERE key = 'today_dish_photo', value ustunida
    """
    conn = get_connection()
    cursor = conn.cursor()
...
def update_today_dish(text, photo):
    """
    Bugungi taom matnini va rasm yo'lini saqlaydi.
    Ikkalasi ham 'value' ustuniga yoziladi (value_photo emas).
    """
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute(
        "INSERT OR REPLACE INTO settings (key, value, value_photo) VALUES ('today_dish_text', ?, NULL)",
        (text,)
    )
    cursor.execute(
        "INSERT OR REPLACE INTO settings (key, value, value_photo) VALUES ('today_dish_photo', ?, NULL)",
        (photo,)
    )
    conn.commit()
    conn.close()
...
# ─────────────────────────────────────────────
# Qiziquvchilar (leads) funksiyalari
# ─────────────────────────────────────────────
